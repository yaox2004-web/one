...
def get_stock_data(market, code):
    ...
    support_dist = (today_price - nearest_support[0]) / today_price * 100 if nearest_support else None
    resistance_dist = (nearest_resistance[0] - today_price) / today_price * 100 if nearest_resistance else None
    
    # 位置判断
    if support_dist is not None and support_dist < 2:
        position_desc = "接近支撑位"
    elif resistance_dist is not None and resistance_dist < 2:
        position_desc = "接近压力位"
    elif support_dist is not None and resistance_dist is not None:
        if support_dist < 5 and resistance_dist < 5:
            position_desc = "在支撑和压力之间（震荡）"
        elif support_dist < resistance_dist:
            position_desc = "偏近支撑位"
        else:
            position_desc = "偏近压力位"
    else:
        position_desc = "无明显关键位"
    
    # ========== 第十步：关键位置形态 ==========
    key_pattern = []
    
    # 在支撑位附近出现阳线放量
    if nearest_support and support_dist < 3:
        if today['close'] > today['open'] and vol_ratio_yesterday > 1.2:
            key_pattern.append("支撑位附近出现阳线放量")
        elif today['close'] < today['open'] and vol_ratio_yesterday < 0.8:
            key_pattern.append("支撑位附近出现阴线缩量")
        elif today['close'] > today['open']:
            key_pattern.append("支撑位附近出现阳线")
    
    # 在压力位附近出现阴线放量
    if nearest_resistance and resistance_dist < 3:
        if today['close'] < today['open'] and vol_ratio_yesterday > 1.2:
            key_pattern.append("压力位附近出现阴线放量")
        elif today['close'] > today['open'] and vol_ratio_yesterday > 1.2:
            key_pattern.append("压力位附近出现阳线放量")
    
    if not key_pattern:
        key_pattern.append("不在关键位置附近")
...
<head>
    ...
    <title>十步客观描述报告 - {today_str}</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .position-badge {{ display: inline-block; padding: 4px 12px; border-radius: 15px; font-size: 12px; font-weight: bold; color: white; margin-top: 5px; }}
        .pos-support {{ background: #388e3c; }}
        .pos-resistance {{ background: #d32f2f; }}
        .pos-middle {{ background: #f57c00; }}
    </style>
</head>
<body>
    <div class="container">
...
"""
    ...
    for stock in stocks_data:
        ...
        # 第九步位置标签
        if "支撑" in stock['position_desc']:
            badge_class = "pos-support"
        elif "压力" in stock['position_desc']:
            badge_class = "pos-resistance"
        else:
            badge_class = "pos-middle"
        
        # 第十步关键形态
        key_pattern_html = ""
        for pattern in stock['key_pattern']:
            key_pattern_html += f'<p>{pattern}</p>'
        ...
        <div class="stock-card">
            ...
            <div class="step-section">
                <div class="step-title">第八步：价格在哪里（第九步）</div>
                <div class="step-content">
                    <span class="position-badge {badge_class}">{stock['position_desc']}</span>
                    <p>离最近支撑位：{stock['support_dist']:.2f}%</p>
                    <p>离最近压力位：{stock['resistance_dist']:.2f}%</p>
                </div>
            </div>
            
            <div class="step-section">
                <div class="step-title">第九步：关键位置形态（第十步）</div>
                <div class="step-content">
                    {key_pattern_html}
                </div>
            </div>
            ...
            <div class="step-section">
                <div class="step-title">第十步：客观总结</div>
                <div class="step-content">
